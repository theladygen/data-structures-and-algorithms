...
const forLoopTwoToThe = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    powerOfTwo.push(Math.pow(2, arr[i]));
  }
  return powerOfTwo;
};
...
const forEachTwoToThe = (arr) => {
  let newPowerOfTwo = [];
  arr.forEach((value) => {
    newPowerOfTwo.push(Math.pow(2, value));
  });
  return newPowerOfTwo;
};
...
const mapTwoToThe = (arr) => {
  let mapPowerOfTwo = [];
  arr.map(value => mapPowerOfTwo.push(Math.pow(2, value)));
  return mapPowerOfTwo;
};
